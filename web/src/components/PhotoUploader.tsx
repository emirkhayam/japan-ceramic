...
export function PhotoUploader({ value, onChange }: Props) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        <div className="mb-2 text-xs uppercase tracking-wider text-mist-400">
          Или попробуйте на нашей комнате
        </div>
        <div className="grid grid-cols-3 gap-2">
          {DEMO_ROOMS.map((room) => (
            <button
              key={room.id}
              onClick={() => onChange(room.src)}
              className="group relative aspect-[4/3] overflow-hidden rounded-xl border border-white/10 transition hover:border-gold-500/50"
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={room.src}
                ...
                onError={(e) => {
                  // Fallback while demo images not yet generated
